...
import type { CostCategory, CostItem, PaymentMethod, PaymentStatus } from '../../domain/models/types';
import { extractCostFromText } from '../../infrastructure/api/geminiApi';
import { Sparkles, Loader2 } from 'lucide-react';
import toast from 'react-hot-toast';

interface CostFormProps {
  onSave: (cost: Omit<CostItem, 'id' | 'projectId'>) => void;
  onCancel: () => void;
...
export const CostForm: React.FC<CostFormProps> = ({ onSave, onCancel, initialData }) => {
    ...
    <Card className="p-6 bg-white border-primary-200 shadow-md">
      ...
      {isEditing && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
          <p className="text-sm font-medium text-amber-800">✏️ Modo de edição — altere os campos e salve.</p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          <Button type="submit">{isEditing ? 'Salvar Alterações' : 'Salvar Custo'}</Button>
        </div>
      </form>
    </Card>
  );
